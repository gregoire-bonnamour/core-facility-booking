...
if not user_profiles or not equipment_set:
    print("⚠️ Pas assez de données existantes (user_profiles et équipements requis).")
else:
    print(f"✅ {len(user_profiles)} user_profiles et {len(equipment_set)} équipements trouvés")
...
for _ in range(nb_reservations):
    ...
    if resa_type == "assistance":
        ...
        participants = random.sample(user_profiles, k=min(len(user_profiles), random.randint(2, 5)))
