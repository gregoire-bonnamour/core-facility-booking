...
Notes :
- L’authentification se fait par email (voir usager.backends.EmailAuthBackend).
...
def confirmer_inscription(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        return redirect('usager:inscription')
...
def inviter_usager(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'relancer':
            ...
            return redirect('usager:inviter_usager')
...
def _envoyer_email_invitation(request, invitation, rappel=False):
    ...
    lien_inscription = request.build_absolute_uri(
        reverse("usager:inscription") + "?" + urlencode({"courriel": invitation.courriel})
    )
...
def profil(request):
    ...
    """
    usager = getattr(request.user, 'usager', None)
    return render(request, 'usager/profil.html', {'user': request.user, 'usager': usager})
...
def _envoyer_reglement_pdf(request):
    ...
    user = request.user
    usager_email = getattr(user, "email", "") or getattr(getattr(user, "usager", None), "courriel", "")
    if not usager_email:
