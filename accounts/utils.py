...
from accounts.models import Invitation, Usager
...
def creer_invitations_pour_formation(reservation, force_resend=False):
    ...
    """
    # On sépare les courriels (virgules, points-virgules, sauts de ligne)
    raw_emails = reservation.courriels_formes or ''
    ...
    existing_emails = set(
        ...
        .values_list('courriel', flat=True)
    )
    ...
    for email in emails:
        ...
        # Mettre à jour l'objet invitation
        Invitation.objects.filter(courriel__iexact=email, reservation=reservation).delete()
        ...
        # 1. Vérifier si l'utilisateur existe déjà pour choisir le type de courriel
        user_exists = User.objects.filter(email__iexact=email).exists()
        usager_exists = Usager.objects.filter(courriel__iexact=email).exists()
        ...
        if user_exists or usager_exists:
            ...
            send_mail(
                subject=f"[Formation] Inscription confirmée – {reservation.equipement.nom}",
                message=(
                    f"Bonjour,\n\n"
                    f"Vous avez été inscrit(e) à une formation sur l’équipement : {reservation.equipement.nom}\n"
                    f"Date : le {reservation.date_debut.strftime('%Y-%m-%d')} "
                    f"de {reservation.heure_debut.strftime('%H:%M')} à {reservation.heure_fin.strftime('%H:%M')}.\n\n"
            ...
            # Lien d'inscription
            inscription_url = f"{settings.SITE_URL}/usager/inscription?courriel={email}"
            ...
            send_mail(
                ...
                message=(
                    f"Bonjour,\n\n"
                    f"Vous avez été invité à suivre une formation sur l’équipement : {reservation.equipement.nom}\n"
                    f"le {reservation.date_debut.strftime('%Y-%m-%d')} à la plateforme de microscopie et cytométrie du YourFacility.\n\n"
...
def est_admin_plateforme(user):
    ...
    Retourne True si l'usager est Super-utilisateur (Django), Staff (Django) 
    OU a la case 'Est admin' cochée sur son profil plateforme.
    """
    return user.is_staff or user.is_superuser or (hasattr(user, 'accounts') and getattr(user.usager, 'est_admin', False))
